...
train_tweets = ptd.getAllTweetsWithoutHashOrAlphaTag(ptd.getAllTweets(ptd.getTopicData(data)))
# Getting all the hashtags from each tweet
train_hashtags = ptd.getAllHashtags(ptd.getAllTweets(ptd.getTopicData(data)))
...
# Training and label set
train = train_tweets
train_labels = ptd.getAllStances(ptd.getTopicData(data))
print "len of train set and labels should be the same: " + str(len(train)) + " == " + str(len(train_labels))

# ************ Creating test set (not used if cross_validation.train_test_split is used below)************
data2 = TOPIC2
test_tweets = ptd.getAllTweetsWithoutHashOrAlphaTag(ptd.getAllTweets(ptd.getTopicData(data2)))
test_hashtags = ptd.getAllHashtags(ptd.getAllTweets(ptd.getTopicData(data2)))
...
test = test_tweets
test_labels = ptd.getAllStances(ptd.getTopicData(data2))
...
os.system("perl eval.pl gold.txt guess.txt")